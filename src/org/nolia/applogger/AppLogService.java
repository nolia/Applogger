...
public class AppLogService extends Service {
    ...
    public static final SimpleDateFormat DATE_FORMAT = new SimpleDateFormat(
            "dd-MM-yyyy-HHmm", Locale.getDefault());
    ...
    public List<String> getRunning() {
        ...
        for (RunningAppProcessInfo info : runningAppProcesses) {
            // if package is used as process name and is launchable - then user
            // started it.
            final boolean visibleApp = (null != pm
                    .getLaunchIntentForPackage(info.processName));
    ...
    private void initThreads() {
        watchThread = new HandlerThread("WatchThread",
                Process.THREAD_PRIORITY_DEFAULT);
        watchThread.start();
        watchHandler = new Handler(watchThread.getLooper(), watchCallback);

        fileThread = new HandlerThread("LogFileThread",
    ...
    private boolean initFile() {
        try {
            ...
            logFile = (extStorageAccessible) ? createLogFile()
                    : createLocalFile();
    ...
    private File createLogFile() throws IOException {
        File logsDir = new File(Environment.getExternalStorageDirectory()
                .getAbsolutePath() + File.separatorChar + APPS_LOG_DIR);
        if (!logsDir.exists()) {
            if (!logsDir.mkdirs()) {
    ...
    private Notification createNotification(boolean show) {
        ...
        notification.setLatestEventInfo(this, "Logging apps",
                "Apps started to be logged.", contentIntent);
        return notification;
    }
    ...
    private String timeToString(long time) {
        return (time == AppLogService.TIME_UNKNOWN) ? " - " : AppLogService.SDF
    ...
    private void processApps() {
        ...
        for (String app : lastApps) {
            if (current.contains(app)) {
                ...
                final Message msg = logHandler.obtainMessage(CODE_SAVE_LOG,
                        appLogInfo);
